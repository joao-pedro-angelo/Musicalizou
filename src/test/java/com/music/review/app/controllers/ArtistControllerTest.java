...
class ArtistControllerTest {
    ...
    void setUp() {
        ...
        System.out.println("Artista salvo com ID: " + this.artist.getId());
    }
    ...
    void createArtistSucess() throws Exception {
        ArtistCreateDTO artistCreateDTO = new ArtistCreateDTO("Artista Random", "2020", "Brasil", "Bio do artista");
    ...
    void findArtistByIdSucess() throws Exception {
        var response = this.mockMvc.perform(get("/v1/artists/id/{id}", this.artist.getId())
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    void findArtistByNameSucess() throws Exception {
        var response = this.mockMvc.perform(get("/v1/artists/name/{name}", this.artist.getName())
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void updateArtistSucess() throws Exception {
        ArtistUpdateDTO artistUpdateDTO = new ArtistUpdateDTO(this.artist.getId(), "Artista Atualizado", "2021", "EUA", "Nova bio do artista");
    ...
    void deleteArtistSucess() throws Exception {
        var response = this.mockMvc.perform(delete("/v1/artists/{id}", this.artist.getId())
                        .contentType(MediaType.APPLICATION_JSON))
                .andReturn().getResponse();
